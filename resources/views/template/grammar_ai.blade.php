...
<div class="response" >
    ...
    <a class="btn btn-primary btn-sm float-left" href="#!" id="regenerate">{{ __('Generate') }}</a>
...
<script>
    $('body').ready(function() {
        ...
        if ($('.grammer_textarea').length > 0) {
            ...
            var summernoteValue = tinymce.get('pc_demo1').getContent({format:"text"});
        }
        else {
    ...
    function copyGrammerText() {
        ...
        if ($('.grammer_textarea').length > 0) {
            ...
            tinymce.get('pc_demo1').setContent(copied);
        } else {
