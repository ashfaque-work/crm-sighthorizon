        ...
        function fillEventType(request_type, selected = 0) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data != '') {
                        $("#event_type").html(
                            '<option value="0" selected="selected">{{ __('Select Event Type') }}</option>');
                        $.each(data, function(key, data) {
                            var sel = '';
                            if (key == selected) {
                                sel = 'selected';
                            }
                            $("#event_type").append('<option value="' + key + '" ' + sel + '>' + data +
                                '</option>');
                        });
                    } else {
                        $("#event_type").html(
                            '<option value="" selected="selected">{{ __('Select Event Type') }}</option>');
                    }
            ...
            @can('Request MDF')
                <div class="col-auto pe-0">
                    <a href="#" class="btn btn-sm btn-primary btn-icon" data-bs-toggle="tooltip" data-bs-placement="top"
                        title="{{ __('MDF Request') }}" data-ajax-popup="true" data-size="md"
                        data-title="{{ __('Make MDF Request') }}" data-url="{{ route('mdf.create') }}"><i
                            ...
                            <tbody>
                                @foreach ($mdfs as $mdf)
                                    <tr>
                                        <td class="Id">
                                            @can('View MDF')
                                                @if (!empty($mdf->approvedAmt) || Auth::user()->type == 'Owner')
                                                    <a href="{{ route('mdf.show', $mdf->id) }}"
                                                        class="btn  btn-outline-primary"> <i class="fas fa-file-invoice"></i>
                                                        {{ Auth::user()->mdfNumberFormat($mdf->mdf_id) }}</a>
                                                @else
                                        ...
                                        <td>{{ \Auth::user()->priceFormat(!empty($mdf->approvedAmt->amount) ? $mdf->approvedAmt->amount : 0) }}
                                        </td>
                                            ...
                                            <td class="Action">
                                                <span>
                                                            ...
                                                            @if (!empty($mdf->approvedAmt))
                                                                <div class="action-btn bg-success ms-2">
                                                                    <a href="#" class="btn btn-icon btn-sm"
                                                                        ...
                                                                        title="{{ __('Approved') }}" data-toggle="tooltip"
                                                                        data-original-title="{{ __('Approved') }}"><i
                                                                            class="ti ti-circle-check text-white"></i></a>
                                                                </div>
                                                            @else
                                                                <div class="action-btn bg-info ms-2">
                                                                    <a href="#" data-size="lg"
                                                                        data-url="{{ route('mdf.payments.approved', [$mdf->id, 'approved']) }}"
                                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                                        title="{{ __('Approved Amount') }}"
                                                                        data-ajax-popup="true"
                                                                        data-title="{{ __('Approved Amount') }}"
                                                                        data-toggle="tooltip"
                                                                        data-original-title="{{ __('Approved Amount') }}"
                                                    ...
                                                    @if ($mdf->is_complete == '0')
                                                        @can('Edit MDF')
                                                            <div class="action-btn bg-info ms-2">
                                                                <a href="#" data-url="{{ route('mdf.edit', $mdf->id) }}"
                                                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                                                    title="{{ __('Edit MDF') }}" data-ajax-popup="true"
                                                                    data-title="{{ __('Edit MDF') }}"
                                                                    class="btn btn-icon btn-sm"><i
