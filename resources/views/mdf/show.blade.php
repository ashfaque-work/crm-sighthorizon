...
@section('action-button')
                ...
                @if($mdf->is_complete == '0')
                    ...
                    @can('Create MDF Payment')
                            <a href="#" class="btn btn-sm btn-primary btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('Add Fund')}}" data-ajax-popup="true" data-size="lg" data-title="{{__('Add Fund')}}" data-url="{{ route('mdf.payments.approved',[$mdf->id,'approved'])}}"><i class="ti ti-plus text-white"></i></a>
                    @endcan
                        ...
                        @if(\Auth::user()->type == 'Owner')
                            <div class="col-12 text-end py-2">
                                ...
                                @if($mdf->is_complete == 1)
                                    <a href="#" class="badge p-2 px-3 rounded bg-warning" data-toggle="tooltip" data-original-title="{{__('Mark as Pending this MDF')}}" data-confirm="{{__('Are You Sure?')}}|{{__('This action Mark Your MDF as a "Complete"')}}" data-confirm-yes="document.getElementById('change-status-form-{{$mdf->id}}').submit();"><i class="fas fa-clock"></i> <span>{{__('Pending')}}</span></a>
                                @else
                                    <a href="#" class="badge p-2 px-3 rounded bg-success" data-toggle="tooltip" data-original-title="{{__('Mark as Complete this MDF')}}" data-confirm="{{__('Are You Sure?')}}|{{__('This action Mark Your MDF as a "Pending"')}}" data-confirm-yes="document.getElementById('change-status-form-{{$mdf->id}}').submit();"><i class="fas fa-check"></i> <span>{{__('Complete')}}</span></a>
                                @endif
                                {{ Form::close() }}
                            </div>
                        @endif
