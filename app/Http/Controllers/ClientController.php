...
class ClientController extends Controller
{
        ...
        if(\Auth::user()->can('Create Client'))
        {
            ...
            $client = User::create(
                [
                    ...
                    'type' => 'Client',
                    'lang' => !empty($setting['default_language']) ? $setting['default_language'] : 'en',
    ...
    public function fileImport(Request $request)
    {
        ...
        for ($i = 1; $i <= count($clients) - 1; $i++) {
            $client          = $clients[$i];
            $clientByEmail   = User::where('email', $client[2])->first();

            if (!empty($clientByEmail)) {
                $errorArray[$i] = $clientByEmail->email;
            } else {
                $clientData = new User();
                ...
                $clientData->is_active = (int)$client[6];
                $clientData->messenger_color = $client[9];
                $clientData->save();
            }
        }
        if (!empty($errorArray)) {
            $data['status'] = 'error';
            $data['msg'] = $totalclient . '  ' . __('Record imported fail out of' . ' ' . count($errorArray) . ' ' . 'record');
        } else {
            $data['status'] = 'success';
