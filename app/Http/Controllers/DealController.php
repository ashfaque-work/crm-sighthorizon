...
class DealController extends Controller
{
    ...
    public function deal_list()
    {
        ...
        if($usr->can('Manage Deals'))
        {
            ...
            $deals = Deal::with('users','clients','pipeline')->where('created_by', \Auth::user()->ownerId())->get();
            $curr_month  = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->whereMonth('created_at', '=', date('m'))->get();
            $curr_week   = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->whereBetween(
                'created_at', [
                                \Carbon\Carbon::now()->startOfWeek(),
        ...
        if($usr->can('Create Deal'))
        {
            ...
            else
            {
                ...
                foreach(array_keys($clients) as $client)
                {
                    ...
                    {
                        Utility::sendNotification('assign_deal', $client, $dealArr);
                    }
                ...
                $clients = User::whereIN('id', array_filter($request->clients))->get()->pluck('name')->toArray();
                ...
                if(isset($settings['deal_notification']) && $settings['deal_notification'] ==1){
                    \Utility::send_slack_msg('new_deal',$obj);
                }
                if(isset($settings['telegram_deal_notification']) && $settings['telegram_deal_notification'] ==1){
                    \Utility::send_telegram_msg('new_deal',$obj);
                }
                $module ='deal create';
    ...
    public function show(Deal $deal)
    {
        if($deal->is_active)
        {
            ...
            {
                $permission = \Auth::user()->clientPermission($deal->id);
                if($permission)
                {
            ...
            return view('deals.show', compact('deal','transdate','customFields', 'calenderTasks', 'permission'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function order(Request $request)
    {
        ...
        if($usr->can('Move Deal'))
        {
            ...
            if($deal->stage_id != $post['stage_id'])
            {
                ...
                foreach(array_keys($usrs) as $key)
                {
                    Utility::sendNotification('move_deal', $key, $dealArr);
                }
            ...
            if($deal->created_by == $usr->ownerId())
            {
                if(!empty($request->users))
                {
                    ...
                    foreach(array_keys($users) as $user)
                    {
                        ...
                        Utility::sendNotification('assign_deal', $user, $dealArr);
                    }
    ...
    public function productEdit($id)
    {
        if(\Auth::user()->can('Edit Deal'))
        {
            ...
            if($deal->created_by == \Auth::user()->ownerId())
            {
                $products = Product::where('created_by', '=', \Auth::user()->ownerId())->whereNOTIn('id', explode(',', $deal->products))->get()->pluck('name', 'id');
        ...
        if($usr->can('Edit Deal'))
        {
            ...
            if($deal->created_by == $usr->ownerId())
            {
                if(!empty($request->products))
                {
                    ...
                    foreach(array_merge($deal_users, $clients) as $key)
                    {
                        Utility::sendNotification('add_product', $key, $productArr);
                    }
                }
        ...
        if(\Auth::user()->can('Edit Deal'))
        {
            ...
            if($deal->created_by == \Auth::user()->ownerId())
            {
                ...
                if($file)
                {
                    ...
                    //               ]
                    // );
                    $logo=Utility::get_file('deal_files/');
    ...
    public function taskStore($id, Request $request)
    {
        ...
        if($usr->can('Create Task'))
        {
            ...
            if($deal->created_by == $usr->ownerId())
            {
                ...
                foreach(array_keys($usrs) as $key)
                {
                    Utility::sendNotification('create_task', $key, $taskArr);
                }
    ...
    public function sourceUpdate($id, Request $request)
    {
        ...
        if($usr->can('Edit Deal'))
        {
            ...
            if($deal->created_by == $usr->ownerId())
            {
                ...
                foreach(array_merge($deal_users, $clients) as $key)
                {
                    Utility::sendNotification('update_deal_source', $key, $dealArr);
                }
    ...
    public function permission($id, $clientId)
    {
        if(\Auth::user()->can('Edit Deal'))
        {
            ...
            $permissions = \App\Models\Deal::$permissions;
    ...
    public function discussionStore($id, Request $request)
    {
        ...
        if($deal->created_by == \Auth::user()->ownerId())
        {
            ...
            foreach(array_merge($deal_users, $clients) as $key)
            {
                Utility::sendNotification('add_discussion', $key, $dealArr);
            }
    ...
    public function callStore($id, Request $request)
    {
        ...
        if($usr->can('Create Deal Call'))
        {
            ...
            if($deal->created_by == $usr->ownerId())
            {
                ...
                Utility::sendNotification('create_deal_call', $request->user_id, $dealArr);
    ...
    public function fileImport(Request $request)
    {
        ...
        for ($i = 1; $i <= count($deals) - 1; $i++) {
            ...
            $dealData->name = $deal[1];
            $dealData->price = $deal[2];
            $dealData->pipeline_id = $deal[3];
            $dealData->Stage_id = $deal[4];
            $dealData->status = $deal[5];
            $dealData->phone = $deal[6];
            $dealData->created_by = \Auth::user()->ownerId();
            $dealData->is_active = (int)$deal[8];
