...
class HomeController extends Controller
{
    ...
    public function index()
    {
           ...
           /* $roles = Auth::user()->roles;*/
            ...
            elseif($user->type == 'Owner')
            {
                ...
                if (is_null(self::$user)) {
                    ...
                    $arrCount['user']    = User::where('type', '!=', 'Client')->where('created_by', '=', $user->ownerId())->count();
                    self::$user =  $arrCount['user'];
                }
                if (is_null(self::$deal)) {

                    $arrCount['deal']    = Deal::where('created_by', '=', $user->ownerId())->count();
                    self::$deal = $arrCount['deal'];
                }
                ...
                if (is_null(self::$user)) {
                    ...
                    self::$user =   $chartcall['user'];
                }
                if (is_null(self::$deal)) {

                           $chartcall['deal']    = Deal::where('created_by',$user->ownerId())->count();
                    self::$deal = $chartcall['deal'];
                }

                        $stageNames = Stage::pluck('name');

                        // Retrieve stages using the collected names in a single query
                        $stages = Stage::whereIn('name', $stageNames)->get();

                        // Organize the stages in the desired format (associative array)
                        $stagedata['stage'] = $stages;

                        // Now $stagedata['stage'] contains all the stages with the specified names


                if (is_null(self::$user)) {

               $stagedata['user']    = User::where('type', '!=', 'Client')->where('created_by',$user->ownerId())->count();
                    self::$user =    $stagedata['user'];
                }
                if (is_null(self::$deal)) {

                   $stagedata['deal']    = Deal::where('created_by',$user->ownerId())->count();
                    self::$deal = $stagedata['deal'];
                }
            ...
            elseif($user->type == 'Client')
            {
                ...
                for($i = 0; $i <= 7 - 1; $i++)
                {
                    ...
                    $arrTemp['date'][]    = __(date('d-M', strtotime($date)));
                    // $arrTemp['invoice'][] = Invoice::where('issue_date', '=', $date)->where('created_by', '=', $user->ownerId())->count();
                    $arrTemp['invoice'][]    = Invoice::select('invoices.*')
                                    ->join('deals', 'invoices.deal_id', '=', 'deals.id')
                                    ->join('client_deals', 'client_deals.deal_id', '=', 'deals.id')
                                    ->whereDate('invoices.issue_date', '=', $date)
                                    ->where('client_deals.client_id', '=', \Auth::user()->id)
                                    ->where('invoices.created_by', '=', \Auth::user()
                                    ->ownerId())
                                    ->count();

                    // $arrTemp['payment'][] = InvoicePayment::join('invoices', 'invoice_payments.invoice_id', '=', 'invoices.id')->where('invoice_payments.date', '=', $date)->where('invoices.created_by', '=', $user->ownerId())->count();
                    $arrTemp['payment'][]    = InvoicePayment::select(['invoice_payments.*'])
                                    ->join('invoices', 'invoice_payments.invoice_id', '=', 'invoices.id')
                                    ->join('client_deals', 'client_deals.deal_id', '=', 'invoices.deal_id')
                                    ->whereDate('invoice_payments.date', '=', $date)
                ...
                for($i = 0; $i <= 7 - 1; $i++)
                {
                    ...
                    $temp['deal'][] =  Deal::select('deals.*')
                                        ->join('client_deals', 'deals.id', '=', 'client_deals.deal_id')
                                        ->whereDate('deals.created_at',$date)
                                        ->where('deals.created_by', $user->ownerId())
                                        ->where('client_deals.client_id', \Auth::user()->id)
                                        ->count();
                ...
                $arrCount['invoice']    = Invoice::select('invoices.*')->join('deals', 'invoices.deal_id', '=', 'deals.id')->join('client_deals', 'client_deals.deal_id', '=', 'deals.id')->where('client_deals.client_id', '=', \Auth::user()->id)->count();
            ...
            $deals       = Deal::whereIn('id', $id_deals)->take(5)->get();
        ...
        else
        {
            ...
            {
                if(Utility::getValByName('enable_landing') == 'yes' && \Schema::hasTable('landing_page_settings'))
                {
