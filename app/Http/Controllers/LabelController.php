...
class LabelController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('Create Label'))
        {
            ...
            $colors = Label::$colors;
            $category = ['Lead Source', 'Call Status', 'Kind of Lead']; // the index here should not be changed
            return view('labels.create')->with('pipelines', $pipelines)->with('colors', $colors)->with('category', $category);
        }
        else
        {
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('Create Label'))
        {
            ...
            if ($validator->fails()) {
        return redirect()->route('labels.index')->withErrors($validator)->withInput()->with('error', __('Validation failed. Please check the form.'));
    }
            if($validator->fails())
            {
            ...
            $category = ['Lead Source', 'Call Status', 'Kind of Lead'];
            $label              = new Label();
            $label->name        = $request->name;
            $label->color       = $request->color;
            $label->category    = $category[$request->category];
    ...
    public function edit(Label $label)
    {
        if(\Auth::user()->can('Edit Label'))
        {
            if($label->created_by == \Auth::user()->ownerId())
            {
                ...
                return view('labels.edit', compact('label', 'pipelines', 'colors', 'category'));
            }
            else
            {
                return response()->json(['error' => __('Permission Denied.')], 401);
